...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const hasPermission =
      permissions.length === 0 || permissions.every((permission) => user.permissions.includes(permission));

    if (!hasRole || !hasPermission) {
      throw new ForbiddenException('Permissao insuficiente para executar esta acao.');
    }
